...
/**
 * Tests payment expirations.
 *
 * @group commerce
 */
class PaymentMethodReusableTest extends CommerceKernelTestBase {
  ...
  protected function setUp() {
    ...
    $this->paymentMethodStorage = $this->container->get('entity_type.manager')->getStorage('commerce_payment_method');
  }

  /**
   * Tests loading active and expired payment methods.
   */
  public function testExpiredPaymentMethods() {
    ...
    $this->assertNotEmpty(count($this->paymentMethodStorage->loadMultiple()) == 2);
    $this->assertNotEmpty(count($this->paymentMethodStorage->loadReusable($this->user, $this->paymentGateway)) == 1);
  }

  /**
   * Test reusable payment methods and store billing country restrictions.
   */
  public function testBillingCountryPaymentMethods() {
    $this->store->setBillingCountries(['US']);
    $this->store->save();
    ...
    $payment_method_fr->setBillingProfile($profile_fr);
    $payment_method_fr->save();

    $this->assertEmpty($this->paymentMethodStorage->loadReusable($this->user, $this->paymentGateway, $this->store));
    ...
    $payment_method_us->setBillingProfile($profile_us);
    $payment_method_us->save();

    $this->assertNotEmpty($this->paymentMethodStorage->loadReusable($this->user, $this->paymentGateway, $this->store));
  }
