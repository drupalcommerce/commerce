...
class PaymentMethodAddForm extends FormBase implements ContainerInjectionInterface {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, UserInterface $user = NULL) {
    ...
    if (!$payment_gateway) {
      ...
      if (!$payment_gateway || !($payment_gateway->getPlugin() instanceof SupportsStoredPaymentMethods)) {
    ...
    if (!$step) {
      $step = 'select_payment_method_type';
      ...
      if (count($payment_method_types) === 1) {
        ...
        $step = 'add_payment_method';
      }
      $form_state->set('step', $step);
    }

    if ($step == 'select_payment_method_type') {
      $form = $this->buildSelectPaymentMethodTypeForm($form, $form_state);
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($step == 'select_payment_method_type') {
      ...
      $form_state->set('step', 'add_payment_method');
      $form_state->setRebuild(TRUE);
    }
    elseif ($step == 'add_payment_method') {
      /** @var PaymentMethodInterface $method */
      $method = $form_state->getValue('payment_method');
      drupal_set_message($this->t('%label saved to your payment methods.', ['%label' => $method->label(),]));
      $form_state->setRedirect('entity.commerce_payment_method.collection', ['user' => $method->getOwnerId()]);
    }
  }
